...
INSERT INTO Currency (name, abbreviation)
VALUES 
("dolars","USD"),
("euros", "EUR"),
("colombian pesos", "COP");
...
BEGIN
    ...
    DECLARE countable_id INT UNSIGNED DEFAULT 0;
    DECLARE purchasable_id INT UNSIGNED DEFAULT 0; 
    ...
    SET countable_id =LAST_INSERT_ID();
	INSERT INTO Purchasable (price_per_unit, Countable_id, Currency_id)
	VALUES (p_price_per_unit, countable_id,1);
	SET purchasable_id =LAST_INSERT_ID();
    
    -- At the end we commit, if there are not errors.
    COMMIT;
    
    SELECT countable_id, purchasable_id;

END$$

CREATE PROCEDURE InsertPurchasableOff(
    IN p_name VARCHAR(20),
    IN p_quantity DECIMAL(6,2),
    IN p_price_per_unit DECIMAL(10, 2),
    IN p_Farms_id TINYINT
...
BEGIN
    ...
    CALL InsertPurchasableOff(p_name,p_quantity,p_price_per_unit,p_Farms_id);
...
CREATE PROCEDURE InsertPurchasableProduct(
    IN p_name VARCHAR(20),
    IN p_quantity DECIMAL(6,2),
    IN p_price_per_unit DECIMAL(10, 2),
    IN p_Farms_id TINYINT,
...
BEGIN
	...
	CALL InsertPurchasableOff(p_name,p_quantity,p_price_per_unit,p_Farms_id);
    CALL InsertProduct(p_sell_price,p_rotten_percentage,p_rot_per_day);
...
CALL InsertPurchasableOff("Corn Product",100,1,1);
CALL InsertProduct(2,0,0.5);
CALL InsertPurchasableOff("Quinoa Product",200,2.5,1);
CALL InsertProduct(3,0,0.2);


-- Insert tool (wear_and_tear_percentage, wear_and_tear_per_day)
CALL InsertPurchasableOff("Tractor",2,20000,1);
CALL InsertTool(0,1);
CALL InsertPurchasableOff("Harvester",1,10000,1);
CALL InsertTool(0,1);
CALL InsertPurchasableOff("Plow",2,5000,1);
CALL InsertTool(0,1);
CALL InsertPurchasableOff("Disc Harrow",1,4000,1);
CALL InsertTool(0,1);
CALL InsertPurchasableOff("Seeder",5,15000,1);
CALL InsertTool(0,1);
CALL InsertPurchasableOff("Wheelbarrow",2,100,1);
CALL InsertTool(0,1);
CALL InsertPurchasableOff("Shovel",3,35,1);
CALL InsertTool(20,0.5);
CALL InsertPurchasableOff("Pitchfork",1,50,1);
CALL InsertTool(0,2);
...
CALL InsertPurchasableOff("Cows",3,1800,1);
CALL InsertAnimal(LAST_INSERT_ID(),1,1,GetAnimalTypeId("Cattle"));
CALL InsertPurchasableOff("Chikens",20,8,1);
CALL InsertAnimal(LAST_INSERT_ID(),2,2,GetAnimalTypeId("Chickens"));
CALL InsertPurchasableOff("Bee Hive",2,300,1);
CALL InsertAnimal(LAST_INSERT_ID(),3,3,GetAnimalTypeId("Bees"));

-- Insert purchasable (name, quantity, price_per_unit, Farms_id)
-- Insert product (sell_price, rotten_percentage, rot_per_day)
CALL InsertPurchasableProduct("Rice Product",100,1,1,2,0,0.5);
